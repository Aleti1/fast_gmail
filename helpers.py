...
class Creds:
    def __init__(
		...
		secret_file_path: Optional[str] = "credentials.json",
		application_type: ApplicationType = ApplicationType.WEB
    ) -> None:

        self.client_secret_file = secret_file_path
        self.creds = json_creds
        if json_creds:
            self.creds = str(json_creds)
        self.flow = None
        with open(self.client_secret_file, "+r") as f:
            self.data = json.loads(f.read())
            self.redirect_url: str = self.data[application_type.value]["redirect_uris"][0]

    def get_credentials(self) -> Credentials:
        if self.creds:
            return Credentials.from_authorized_user_info(ast.literal_eval(self.creds))
    ...
    def get_json_creds(self) -> str:
        return self.flow.credentials.to_json()

    def refresh_token(self) -> dict:
        self.flow.credentials.refresh(Request())
        return {
            "refresh_token": self.flow.credentials.refresh_token, 
            "access_token": self.flow.credentials.to_json()
        }

    def create_credentials(self, code=None):
        if not code:
            url = self.get_auth_token_url()
            return url
        try:
            self.flow = Flow.from_client_secrets_file(
                self.client_secret_file,
            ...
            self.creds = self.flow.credentials.to_json()
