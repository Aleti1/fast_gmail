...
class GmailApi(object):
        ...
        if auth_token:
            try:
                self.credentials = Credentials.from_authorized_user_info(
                    json.loads(auth_token), SCOPES
                )
            except Exception:
                raise InvalidToken("INVALID AUTH TOKEN PROVIDED")
        self.auth_url: str = ""
            ...
            if (
                ...
                match application_type:
                    ...
                    case ApplicationType.WEB:
                        ...
                        if "web" not in cred_payload:
                            raise Exception(
                                f"{cred_payload=} content missing."
                                "\n\033[91mDid you create credentials for Web app?\033[0m"
                            )
                        if (
                            "redirect_uris" not in cred_payload["web"]
                            or not cred_payload["web"]["redirect_uris"]
                        ):
                            raise Exception(
                        ...
                        if code:
                            ...
                            self.auth_url = (
                                f"{self.flow.authorization_url()[0]}&prompt=consent"
                            )
                            return
    ...
    def get_message(self, id: str) -> Optional[Message]:
        ...
        return Message(
            ...
            **self.google_service.service.users()
            .messages()
            .get(userId=self.google_service.user_id, id=id)
            .execute(),
        )
    ...
    def send_message(
        ...
        try:
            ...
            send_msg = (
                self.google_service.service.users()
                .messages()
                .send(userId=self.google_service.user_id, body={"raw": encoded_message})
                .execute()
            )
        except HttpError as e:
            raise e
    ...
    def labels(self) -> Optional[List[GmailLabel]]:
        try:
            results = (
                self.google_service.service.users()
                .labels()
                .list(userId=self.google_service.user_id)
                .execute()
            )
        except HttpError as e:
            raise e
    ...
    def profile(self) -> Optional[GmailProfile]:
        ...
        try:
            request = (
                self.google_service.service.users()
                .getProfile(userId=self.google_service.user_id)
                .execute()
            )
    ...
    def _alias_info(self, email_address: str) -> dict:
        ...
        Returns:
            dict: A dictionary containing information about the alias, or an empty dictionary if the alias is not found.
        ...
        try:
            return (
                ...
                .settings()
                .sendAs()
                .get(sendAsEmail=email_address, userId=self.google_service.user_id)
                .execute()
            )
        except HttpError as e:
            raise e
    ...
    def _get_messages(
        ...
        if previous_page_token:
            ...
            if previous_page_token not in existing_pages:
                raise InvalidToken(
                    f"{previous_page_token=} not found in {existing_pages=}"
                )
        ...
        try:
            ...
            def on_get_messages_callback(request_id, response, exception) -> None:
                ...
                results.append(
                    BatchResponse(
                        message=Message(google_service=self.google_service, **response)
                    )
                )
            ...
            for index in range(0, len(messages_response.drafts)):
                draft_id = DraftIdentifiers(
                    ...
                    message=MessageIdentifiers(
                        **messages_response.drafts[index]["message"]
                    ),
                )
                batch.add(
                    self.google_service.service.users()
                    .drafts()
                    .get(userId=self.google_service.user_id, id=draft_id.id),
    ...
    def get_contacts(self) -> Optional[list]:
        try:
            request = (
                ...
                .otherContacts()
                .list(pageSize=1000, readMask="emailAddresses")
                .execute()
            )
    ...
    def search_contacts(self, query: str) -> Optional[list]:
        try:
            request = (
                build("people", "v1", credentials=self.credentials)
                .otherContacts()
                .search(pageSize=30, readMask="emailAddresses", query=query)
                .execute()
            )
